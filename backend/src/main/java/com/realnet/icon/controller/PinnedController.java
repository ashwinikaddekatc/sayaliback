...
public class PinnedController {
	...
	public ResponseEntity<?> savePinned(@RequestBody PinnedEntity pinned, HttpSession session1) throws IOException {
		...
		Optional<PinnedEntity> age = pinnedRepository.findobject(pinned.getObjectId(),
		...
		pinned.setUserId(loggedInUser.getUserId());
	...
	@ApiOperation(value = "Delete A pinned", response = PinnedEntity.class)
		@DeleteMapping("/removePinById/{object_id}")
		public void deleteFavourite(@PathVariable(value = "object_id") int object_id) {

		AppUser loggedInUser = userService.getLoggedInUser();
		Optional<PinnedEntity> age = pinnedRepository.findobject(object_id, loggedInUser.getUserId());
		if (age.isPresent()) {

			pinnedRepository.delete(age.get());
		}
	}
