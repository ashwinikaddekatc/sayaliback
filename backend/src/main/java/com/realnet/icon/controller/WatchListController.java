...
public class WatchListController {
	...
	public ResponseEntity<?> saveWatchList(@RequestBody WatchListEntity watch, HttpSession session1)
			throws IOException {
		AppUser loggedInUser = userService.getLoggedInUser();
		Optional<WatchListEntity> age = watchListRepository.findobject(watch.getObjectId(),
		...
		watch.setUserId(loggedInUser.getUserId());
	...
	@DeleteMapping("/removeWatchlistById/{object_id}")
	public void deleteStarred(@PathVariable(value = "object_id") int object_id) {
		AppUser loggedInUser = userService.getLoggedInUser();
		Optional<WatchListEntity> age = watchListRepository.findobject(object_id, loggedInUser.getUserId());
		if (age.isPresent()) {

			watchListRepository.delete(age.get());
		}
	}
