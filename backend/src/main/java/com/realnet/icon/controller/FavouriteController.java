...
//	    // SAVE
	...
	public ResponseEntity<?> saveFavourite(@RequestBody FavouriteEntity favourite, HttpSession session1)
			throws IOException {
		AppUser loggedInUser = userService.getLoggedInUser();
		Optional<FavouriteEntity> age = favouriteRepository.findobject(favourite.getObjectId(), loggedInUser.getUserId());
		...
		favourite.setUserId(loggedInUser.getUserId());
	...
	@DeleteMapping("/removeFavById/{object_id}")
	public void deleteFavourite(@PathVariable(value = "object_id") int object_id) {
		AppUser loggedInUser = userService.getLoggedInUser();
		Optional<FavouriteEntity> fav = favouriteRepository.findobject(object_id, loggedInUser.getUserId());
		if (fav.isPresent()) {
	...
	// GET BY created_by
	@ApiOperation(value = "Get All favourite by created_by", response = FavouriteEntity.class)
	@GetMapping("/getMyFavPrjs/{id}")
	public ResponseEntity<?> getMyFavouritesprj(@PathVariable(value = "id") Long created_by) {

		List<FavouriteEntity> fav = favService.getAllByCreatedBy(created_by);
		return new ResponseEntity<List<FavouriteEntity>>(fav, HttpStatus.OK);
	}
