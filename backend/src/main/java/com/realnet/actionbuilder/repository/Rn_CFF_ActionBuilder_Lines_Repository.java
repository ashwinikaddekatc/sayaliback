...
public interface Rn_CFF_ActionBuilder_Lines_Repository extends JpaRepository<Rn_cff_ActionBuilder_Lines, Integer> {
	...
	@Query(value = "SELECT * from realnet_CNSBE.rn_cff_action_builder_lines_t WHERE HEADER_ID=:headerId", nativeQuery = true)
	...
	@Query(value = "SELECT * from realnet_CNSBE.rn_cff_action_builder_lines_t WHERE HEADER_ID=:headerId ORDER BY SEQ", nativeQuery = true)
	List<Rn_cff_ActionBuilder_Lines> findByHeaderIdAndOrderBySeq(@Param("headerId") int headerId);
	
	
	@Query(value = "SELECT * from realnet_CNSBE.rn_cff_action_builder_lines_t WHERE HEADER_ID=:headerId AND SEQ=:seq", nativeQuery = true)
