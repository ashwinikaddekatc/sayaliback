...
public class AppUserLogController {
	...
	public ResponseEntity<?> addOne(@PathVariable("username") String username,@PathVariable("mode")String mode){
		...
		if(a!=null) {
			...
			if(l!=null) {
				...
				Object a1 = new Object() {
					private String message="Will now Stop logging from next login";
	...
	public ResponseEntity<?> stopLog(@PathVariable("username") String username){
		AppUser a = appUserServiceImpl.getByUserName(username).orElse(null);
		DateFormat simple =new SimpleDateFormat("dd MMM yyyy HH:mm:ss");
		if(a!=null) {
			AppUserLog l = loggingService.getOne(a.getUserId());
